      ...
      <Card className="w-full">
        <CardHeader>
          <div className="mb-4 flex gap-2">
            {STEPS.map((_, i) => (
              <div key={i} className={`h-2 flex-1 rounded-full ${i <= step ? 'bg-primary' : 'bg-muted/30'}`} />
            ))}
          </div>
              ...
              {SKILLS.map((s) => (
                <button key={s} onClick={() => toggleSkill(s)}
                  className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                    form.techStack.includes(s) ? 'bg-primary text-primary-foreground' : 'bg-muted/30 text-muted-foreground hover:text-foreground'
                  }`}>
          ...
          {step === 2 && (
            <div className="space-y-3">
              {['Looking to collaborate on projects', 'Open to hiring / being hired', 'Seeking mentorship', 'Exploring the platform'].map((opt) => (
                <label key={opt} className={`flex cursor-pointer items-center gap-3 rounded-lg border p-4 text-sm transition-colors ${form.preferences === opt ? 'border-primary bg-primary/5' : 'border-border hover:bg-muted/30'}`}>
